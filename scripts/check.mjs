...
for (const check of CHECKS) {
  process.stdout.write(`  ${C.dim}…${C.reset} ${check.name.padEnd(PAD)}  ${C.dim}running${C.reset}`);
  const r = await runOne(check);
  results.push(r);
  // Overwrite progress line
  process.stdout.write(`\r  ${statusBadge(r.code)} ${r.name.padEnd(PAD)}  ${C.dim}${fmtMs(r.ms)}${C.reset}\n`);
}

const failed = results.filter((r) => r.code !== 0);

console.log(`\n${C.bold}Summary${C.reset}`);
console.log(`  ${results.length - failed.length}/${results.length} passed  ${C.dim}(${fmtMs(results.reduce((s, r) => s + r.ms, 0))} total)${C.reset}`);
